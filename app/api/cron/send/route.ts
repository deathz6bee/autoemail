...
export async function GET(req: Request) {
  ...
  for (const campaign of campaigns) {
    ...
    // Only lock if status is currently 'scheduled' or 'in_progress', not already 'sending'
    ...
    if (currentCampaign?.status === 'sending') {
      results[campaign.name] = { skipped: 'Already being processed by another cron instance' };
      continue;
    }
